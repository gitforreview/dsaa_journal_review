    ...
    def __init__(self, label_name='two_year_recid', favorable_classes=[0],
        ...
        for i in df['tmp_feature'].unique():
            if 'African' in i and i[-1]=='0':
                df.loc[df['tmp_feature']==i,'mis_prob'] = (random.random()/2)+0.4
            elif 'African' in i:
                df.loc[df['tmp_feature']==i,'mis_prob'] = (random.random()/4)
            elif 'African' not in i and i[-1]=='0':
                df.loc[df['tmp_feature']==i,'mis_prob'] = (random.random()/6)
            else:
                df.loc[df['tmp_feature']==i,'mis_prob'] = random.random()/10
        new_label=[]
