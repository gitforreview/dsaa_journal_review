...
class AdultDataset(StandardDataset):
    ...
    def __init__(self, label_name='income-per-year',
        ...
        df = train
#         N = 20000
#         df['income-per-year']=df['income-per-year'].replace('<=50K.','<=50K')
#         df['income-per-year']=df['income-per-year'].replace('>50K.','>50K')
#         df['income-per-year']
...
#         df = df_result
        ...
        _,df_neg_priv_test =train_test_split(df_neg_priv,test_size=4650,random_state=17)
        _,df_neg_unpriv_test =train_test_split(df_neg_unpriv,test_size=2850,random_state=17)
        ...
        _,df_pos_priv_test =train_test_split(df_pos_priv,test_size=1800,random_state=17)
        _,df_pos_unpriv_test =train_test_split(df_pos_unpriv,test_size=700,random_state=17)
...
def train_before_resample():
    ...
    print("BCR")
    print(get_BCR(dataset_orig_vt.labels.ravel()[orig_sens_att==0],y_pred[orig_sens_att==0],1))
