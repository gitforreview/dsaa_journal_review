...
class CompasDataset(StandardDataset):
    ...
    def __init__(self, label_name='two_year_recid', favorable_classes=[0],
        ...
        df_neg_priv = df_neg.loc[(df_neg['two_year_recid']==1)&(df_neg['race']=='Caucasian'),:]
        df_neg_unpriv = df_neg.loc[(df_neg['two_year_recid']==1)&(df_neg['race']=='African-American'),:]

        _,df_neg_priv_test =train_test_split(df_neg_priv,test_size=460,random_state=10)
        _,df_neg_unpriv_test =train_test_split(df_neg_unpriv,test_size=990,random_state=10)
        ...
        df_pos_priv = df_pos.loc[(df_pos['two_year_recid']==0)&(df_pos['race']=='Caucasian'),:]
        df_pos_unpriv = df_pos.loc[(df_pos['two_year_recid']==0)&(df_pos['race']=='African-American'),:]
        _,df_pos_priv_test =train_test_split(df_pos_priv,test_size=720,random_state=10)
        _,df_pos_unpriv_test =train_test_split(df_pos_unpriv,test_size=830,random_state=10)
        ...
        for i in df['two_year_recid'].unique():
            for j in df['race'].unique():
                ...
                orig_df_nomiss = df.loc[(df['two_year_recid']==i)&(df['race']==j)&(df['priors_count']==3),:]
                ...
                if real_count>= exp_count:
                    ...
                    df_toapp = resample(orig_df_nomiss,replace=True, n_samples=exp_count-real_count,random_state=10)
